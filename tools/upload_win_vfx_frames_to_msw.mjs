...
const assets = Array.from({ length: 8 }, (_, index) => {
  ...
  return [
    `winGlowLoopFrame${String(frameNo).padStart(2, "0")}`,
    `casino_slot_ui_win_glow_loop_frame${frameNo}.png`,
  ];
});
...
async function uploadOne(key, fileName) {
  ...
  const commonArgs = {
    ...
    subcategory: "object",
    name: baseName,
    description: `Frame sprite for the generated casino-style slot win glow VFX loop: ${baseName}.`,
...
await mcp("initialize", {
  ...
  capabilities: {},
  clientInfo: { name: "codex-slot-win-vfx-frame-upload", version: "0.1.0" },
});
...
console.log(`Merged ${assets.length} frame asset(s) into ${manifestPath}`);
