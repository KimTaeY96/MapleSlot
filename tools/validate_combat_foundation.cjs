...
assert(playerSource.includes('@ExecSpace("Server")') && playerSource.includes("ConfirmLadderArrival"), "Only an owning-client arrival RPC may release the server ladder route");
assert(playerSource.includes("senderUserId ~= player.UserId"), "Ladder arrival RPC must validate the owning player");
assert(playerSource.indexOf('self.ActiveLadderKey ~= "" and self.ActiveLadderDestinationLaneKey ~= ""') < playerSource.indexOf("self:UpdateActionPhase(delta)"), "CLIMB must outrank attack and hit action updates");
assert(playerSource.includes("PendingClimbHitReaction"), "Hits received during CLIMB must be queued until arrival");
...
assert.equal((movementDriverSource.match(/controller:ActionClimb\(/g) || []).length, 1, "Native ladder attachment must be entered once per mount sequence");
...
assert(movementDriverSource.includes("autoBattle:ConfirmLadderArrival(autoBattle.LadderMountSequence)"), "Client must confirm arrival only after clamping to the destination Y");
assert(movementDriverSource.includes("PlayLadderAnimationLoop") && movementDriverSource.includes("SpriteAnimClipPlayType.Loop"), "Ladder animation must play as an explicit loop independent of position movement");
assert(movementDriverSource.indexOf('MovementIntent == "CLIMB"') < movementDriverSource.indexOf('ActionPhase == "ATTACK"'), "Client CLIMB must outrank attack and hit presentation");
