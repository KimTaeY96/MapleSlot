...
- 지역별 당첨 배당 데이터는 `Core.xlsx/Paytable.BaseBetRegionIndex`로 분리합니다.
- `Core.xlsx/SlotSymbols`는 전역 기본 심볼 정의입니다. 지역별로 실제 노출 리소스를 바꾸고 싶으면 `ReelStrips`의 `IdleSpriteRuid`, `WinAnimationRuid`, `WinAnimationEnumId`를 수정합니다.
