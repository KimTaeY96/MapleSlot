...
Combat Foundation data is isolated from slot presentation data.

- `CombatConfig`: server tick, player death penalty, revive HP ratio, sandbox control gate, and initial tier.
- `HuntingGroundTiers`: maps `BaseBetRegionIndex` to a player stat profile and monster spawn group.
- `PlayerStatsProfiles`: player HP, attack, movement, critical, and targeting balance.
- `MonsterDefinitions`: monster resource/model identity, combat stats, respawn, and `DropGroupId`.
- `MonsterSpawnGroups`: monster definition, maintained count, spawn anchor, and combat bounds.
...
1. Edit `Combat.xlsx` and `Drop.xlsx` directly.
