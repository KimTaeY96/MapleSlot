...
`ReelStrips.IdleSpriteRuid` 또는 `ReelStrips.WinAnimationRuid`가 비어 있으면 해당 심볼의 `SlotSymbols` 기본 RUID를 사용합니다. 현재 Wild는 모든 BaseBet에서 같은 리소스를 사용하므로 `ReelStrips`의 두 RUID 컬럼을 비워 전역 기본값을 상속합니다. 지역 전용 Wild 리소스를 의도적으로 사용할 때만 해당 릴 셀에 값을 지정합니다.
...
- 지역별 릴 셀 데이터는 `SlotMachine.xlsx/ReelStrips.BaseBetRegionIndex`로 분리합니다.
...
- 전역 기본 리소스를 그대로 사용할 릴 셀은 `IdleSpriteRuid`, `WinAnimationRuid`를 비워 둡니다. 특히 Wild는 전역 리소스 정책이므로 두 컬럼에 중복 RUID를 넣지 않습니다.

## Cheat.xlsx

### CheatCommands

개발/테스트 환경에서만 입력창으로 적용할 수 있는 치트 목록입니다. 이 테이블의 행은 런타임 시작 시 목록 UI에 표시되지만, 실제 효과는 플레이 중 입력창에 코드를 제출한 현재 세션에만 적용됩니다.

- `CheatCommandsIndex`: 치트 행을 식별하는 정수 인덱스입니다.
- `CheatCode`: 테스트 UI 입력창에 입력할 코드입니다. 현재 기본값은 `777`입니다.
- `Description`: 치트 목록에 표시할 설명입니다.
- `CheatType`: 런타임에서 분기할 치트 동작 타입입니다. 현재 `FORCE_777_BONUS_ONCE`를 사용합니다.
- `TargetKey`: 치트가 대상으로 삼는 기능 또는 트리거 키입니다.
- `ForceResultKey`: 강제 결과 키입니다. 777 치트는 `777`을 사용합니다.
- `UseCount`: 코드 입력 1회로 현재 세션에 부여할 사용 가능 횟수입니다.
- `RequiredRuntimeKind`: 치트 허용 런타임 종류입니다. 빌드본에서는 적용되지 않도록 `TEST_SANDBOX`로 둡니다.
- `Enabled`: 목록 노출 및 입력 허용 여부입니다.
- `Notes`: 기획 참고 메모입니다.


---

## Combat Workbook Ownership

Combat Foundation data is isolated from slot presentation data and split by runtime responsibility.

- `Combat.xlsx/CombatConfig`: server tick, player death penalty, revive HP ratio, sandbox control gate, and initial tier.
- `Character.xlsx/PlayerStatsProfiles`: player HP, attack, movement, critical, targeting, `LadderClimbSpeed`, and `LadderExitStandOffset` balance.
- `Monster.xlsx/MonsterDefinitions`: monster resource/model identity, combat stats, respawn, and `DropGroupId`.
- `HuntingGround.xlsx/HuntingGroundTiers`: maps `BaseBetRegionIndex` to a player stat profile and monster spawn group.
...
- `DropGroups`: reusable roll-mode definitions.
- `DropEntries`: typed reward candidates with quantity range and chance/weight fields.

The initial reward type is `CURRENCY`, but the schema and resolver also accept typed `ITEM` grants. Item-key existence validation will be linked when the item catalog is introduced.
...
5. Builders are create-only: existing workbooks are preserved and require an explicit migration before structural regeneration.
