...
Defines player HP, attack power, cadence, attack range, move speed, critical values, and aggro range. It contains balance only, not player appearance.

### MonsterDefinitions

Defines monster resource/model identity, stats, respawn behavior, and `DropGroupId`. The drop group is a string cross-workbook key intentionally independent from row order.

### MonsterSpawnGroups

Defines which monster appears, how many instances are maintained, and which map anchor/bounds are used.
...
- Tier 1 links Base Bet region `1` to the Tier 1 player profile and a single Slime spawn group.
