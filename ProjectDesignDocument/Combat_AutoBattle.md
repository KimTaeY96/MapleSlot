...
- Ladder arrival compares the player Y with the destination lane's `BoundsLeftAnchorKey` platform height. `SpawnAnchorKey` is elevated for monster spawning and must not be used as the floor height. Upward travel releases the ladder only after reaching the platform height so the player cannot fall back while `CombatLaneKey` advances.
