...
- Ladder entry invokes the native climb action once at the player's current world Y, immediately restores that Y after the engine's mount probe, and keeps deterministic manual Y travel at `PlayerStatsProfiles.LadderClimbSpeed`. The built-in `ladder` sprite action is dispatched separately as an explicit loop, so animation advances without handing position ownership back to the native snap path.
- Animation priority is `CLIMB > ATTACK > HIT > IDLE`. Attack and hit requests are deferred while climbing; after the owning client clamps to the exact destination Y, a server arrival RPC releases the climb lock and resumes queued combat decisions.
- The destination is `BoundsLeftAnchorKey` platform Y plus `LadderExitStandOffset`. Crossing above the one-way foothold before entering `IDLE` lets gravity settle the player onto the destination platform instead of releasing the ladder below it.
...
| `CLIMB_LADDER` | Player is aligned with the ladder | Enter the built-in looping `ladder` body action at the current Y, lock all lower-priority actions, and move deterministically at the table-backed climb speed without remounting. | Client clamps above the destination foothold by `LadderExitStandOffset`, confirms arrival to the server, then enters `IDLE` -> `LADDER_EXIT`. |
...
- A new ladder mount sequence calls `ActionClimb` exactly once with a zero probe offset, restores the entry Y, and advances position manually until the client clamps to the destination platform. `ActionStateChangedEvent` plays `ladder` as a separate loop, while `ConfirmLadderArrival` is the only route that releases the server climb lock.
