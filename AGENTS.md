...
- For normal in-game UI, follow the supplied reference as closely as possible and keep the frame plain: simple rounded silver bevels, compact spacing, and restrained highlights. Do not add gemstones, crystals, wings, spikes, blades, heraldic flourishes, fantasy filigree, or ornamental corner pieces unless the user's reference explicitly contains them.
- Gemstones, pointed ornaments, and similarly flashy decorative motifs are reserved for slot-machine-related UI in this project. They must not appear in ordinary RPG windows such as skills, equipment, inventory, quests, character information, tooltips, or quick-slot editors.
